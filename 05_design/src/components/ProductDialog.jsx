...
export default function ProductDialog({ onClose, onSave }) {
  ...
  const handleSubmit = e => {
    e.preventDefault();
    onSave({ name, description, stock: Number(stock), price: Number(price) });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-8 relative">
        ...
        <h2 className="text-lg font-semibold mb-6 text-black">Add New Product</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 mt-6">
            ...
            <button type="submit" className="px-4 py-2 rounded-md bg-black text-white font-medium hover:bg-gray-900">Add Product</button>
          </div>
        </form>
      </div>
    </div>
